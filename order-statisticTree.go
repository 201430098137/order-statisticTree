...
func (t *OrderStatisticTree) LeftRotation(x *Node) {
...
func (t *OrderStatisticTree) RightRotation(x *Node) {
...
func (t *OrderStatisticTree) Insert(z *Node) {
	p := t.Nil
	x := t.Root
	z.size = 1
	// 确认父节点位置
...
// 插入调整
func (t *OrderStatisticTree) InsertFixup(z *Node) {
...
func (t *OrderStatisticTree) Delete(z *Node) {
	deleteColorNode := z
...
// 删除调整
func (t *OrderStatisticTree) DeleteFixup(x *Node) {
...
// replace替换orgin,
func (t *OrderStatisticTree) Transplant(origin *Node, replace *Node) {
...
// 指定子树最大值节点
func (t *OrderStatisticTree) TreeMinimum(n *Node) *Node {
	for n.Left != t.Nil {
		n = n.Left
	}
	return n
...
func (t *OrderStatisticTree) TreeMaximum(n *Node) *Node {
	for n.Right != t.Nil {
		n = n.Right
	}
	return n
...
// 用于按val大小遍历
// 指点节点后继
func (t *OrderStatisticTree) Successor(x *Node) *Node {
	if x.Right != t.Nil {
...
// 指定节点前驱
func (t *OrderStatisticTree) Predecessor(x *Node) *Node {
	if x.Left != t.Nil {
...
// 查找以x为根的树的第i位置节点
func (x *Node) Select(i int) *Node {
	xIndex := x.Left.size + 1
	if i == xIndex {
		return x
	}
	if i < xIndex {
		return x.Left.Select(i)
	}
	return x.Right.Select(i - xIndex)
}

// 返回x节点的位置
func (t *OrderStatisticTree) Rank(x *Node) int {
	xIndex := x.Left.size + 1
	y := x
	for y != t.Root {
		if y == y.Parent.Right {
			xIndex += y.Parent.Left.size + 1
		}
		y = y.Parent
	}
	return xIndex
